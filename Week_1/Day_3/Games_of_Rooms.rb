...
class Game
  ...
  def place2(choose, position)
    if position == "1" && choose.upcase == "B"
      ...
      puts "Se que es una dificl desicion pero debes elegir, yo recomiendo la birra"
      place1(gets.chomp,"1")
    end
  end
  ...
  def place5(choose, position)
    if position == "4" && choose.upcase == "V"
      ...
      puts "No, inoves. Lo mejor es ir a la barra por mas birra"
      place1(gets.chomp,"4")
    end
  end
...
roomArray = [
  ...
  room6 = Room.new(["D"], "Estas en el aseo, escribe D para volver a la sale del DJ")
]
